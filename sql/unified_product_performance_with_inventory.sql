...
-- Single CSV export with sales data + real inventory data
-- =====================================================
...
-- =====================================================
-- FINAL UNIFIED OUTPUT: SALES + INVENTORY IN SINGLE CSV
-- =====================================================
SELECT 
...
-- =====================================================
-- File Name: stock_grip_unified_sales_inventory.csv
-- Update Frequency: Daily
-- Contains: Sales data + Real inventory levels + Business rules
-- Eliminates: 159,595 unit overstock calculations
-- Provides: Accurate 6-week lead time optimization data
-- =====================================================
```

## Key Fields in Your Unified CSV

### Sales Data (from Product Performance):
- `shopify_units_sold`, `shopify_revenue`, `total_marketing_spend`

### Real Inventory Data (from your inventory model):
- `current_inventory_level` - Actual stock from Shopify API
- `available_inventory` - Real available stock
- `unit_cost_gbp` - Real costs from inventory_item table
- `profit_margin_percentage` - Calculated from real costs

### Business Rules (from your inventory model):
- `reorder_point`, `max_stock_level`, `lead_time_days`

### Accurate Calculations:
- `recommended_order_quantity` - Uses REAL inventory, not estimates
- `days_of_stock_remaining` - Based on actual stock levels
- `revenue_at_risk_gbp` - Realistic risk calculations
...
### After (With Unified Model):
- **~270 records** (9 products × 30 days)
- **5-50 units recommended** (realistic quantities)
- **Real inventory levels** (from Shopify API)

This single CSV will provide your Stock GRIP dashboard with accurate data for reliable 6-week lead time decisions.